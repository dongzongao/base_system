<?php
    ...
    require_once IA_ROOT . '/addons/ewei_shopv2/vendor/autoload.php';
    ...
    class Page extends WeModuleSite
    {
        ...
        public function __construct () {
            ...
            $tablePrefix = new \config\TablePrefix('');
            ...
            /*-------------------------------------链接配置 start-----------------------------------------------------------*/
            $isDevMode = true;
            $config = Setup::createXMLMetadataConfiguration(array(__DIR__."/core/src/config/xml"), $isDevMode);
            $conn = [
                'dbname' => '数据库名称',
                'user' => '用户名称',
                'password' => '密码',
            ...
            $data = \Doctrine\ORM\EntityManager ::create ( $conn , $config ,$evm);
            $this->manager = \Doctrine\ORM\EntityManager ::create ( $conn , $config ,$evm);
            $this->con =  \Doctrine\DBAL\DriverManager::getConnection($conn);
            $this->queryBuilder = $this->con->createQueryBuilder ();
            /*-------------------------------------链接配置 end-------------------------------------------------------------*/
